...
let finalObj = {
  Control: {
    show: 12, //  unique id is unique
    like: 32, //subgoal id is unique
    apply: 23, //subgoal id is unique
    visit: 21, //subgoal id is unique
  },
  "Algo 1": {
    show: 12,
    like: 32,
...
console.log(finalObj.Control)

const newData = rawArr.reduce((acc, item) => {
  ...
  if (!acc[variation]) {
    acc[variation] = {
      ...
      apply: 0,
      visit: 0,
      likeSubgoalIds: new Set(),
      applySubgoalIds: new Set(),
      visitSubgoalIds: new Set(),
      showUniqueIds: new Set(),
    };
  }

  if (event === "show_variation" &&
  !acc[variation].showUniqueIds.has(uniqueId)) {
    acc[variation].show++;
    acc[variation].showUniqueIds.add(uniqueId);
  } else if (
    event === "like" &&
    !acc[variation].likeSubgoalIds.has(subgoalId)
  ) {
    acc[variation].like++;
    ...
    event === "apply" &&
    !acc[variation].applySubgoalIds.has(subgoalId)
  ) {
    acc[variation].apply++;

    acc[variation].applySubgoalIds.add(subgoalId);
  } else if (event === "visit" &&
  !acc[variation].visitSubgoalIds.has(subgoalId)) {
    acc[variation].visit++;
    acc[variation].visitSubgoalIds.add(subgoalId);
  }
...
const tableRows = [{
    id:"visit",
    title:"PV incidence",
    subtitle:"Number of PV/total number of searches"
}]
